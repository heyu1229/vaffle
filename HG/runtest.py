...
# my_user='hg.team@heavengifts.com' #收件人邮箱账号
...
def send_mail(file_new,img_new):
    ...
    try:
        ...
        msg['Subject'] = Header('HG正式环境自动化测试报告','utf-8')
        msg.attach(msgText)
...
if __name__ == '__main__':
    ...
    if auto_type =='automation':
        ...
        #查看用例地址中排除下单和取消下单的用例
        discover = unittest.defaultTestLoader.discover(test_dir,pattern='test_1purchase.py')
    elif auto_type =='automatic_order':
        # congratulations成为最新图片
        nowTime = time.strftime ( "%Y%m%d.%H.%M.%S" )
        success_image = Img ().test_success_image ()
        img = Image.open ( success_image + "congratulations.png" )
