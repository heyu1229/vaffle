...
class IOSTest_publish(unittest.TestCase):
    ...
    def testcase001_post_comment(self):
        ...
        #断言是否评论成功
        time.sleep(3)
        self.driver.save_screenshot('..//testreport/screenshot/test005_comments.jpg')
        try:
            self.driver.find_element_by_accessibility_id('1 Comment').is_displayed()  # 点击进入comments列表
            flag=1
        except:
            flag=2
        self.assertEqual(1,flag,self.write.Write_data(1,8,4,'评论失败'))
        self.write.Write_data(1, 8, 4, '评论成功')
